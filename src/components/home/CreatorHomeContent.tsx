...
import { ArrowRight, Briefcase, FileText, Shield, Wallet, MapPin, Heart, IndianRupee, ChevronRight, TrendingUp } from "lucide-react";
...
const CreatorHomeContent = ({ stats, statsLoading, userCity }: CreatorHomeContentProps) => {
  ...
  return (
    <>
        ...
        <div className="grid grid-cols-4 gap-2.5">
          {quickActions.map((action, i) => (
            <Link
              key={i}
              to={action.to}
              className="rounded-2xl p-3 text-center active:scale-90 transition-all duration-150 flex flex-col items-center gap-2 opacity-0 animate-fade-up"
      ...
      {/* Stats Widget — Premium dark card */}
        ...
        <div className="rounded-2xl p-5 bg-[hsl(222,47%,11%)] border border-accent/20 shadow-[0_0_30px_-10px_hsl(262,83%,58%,0.2)]">
          ...
          <svg className="w-full h-8 mt-3" viewBox="0 0 200 30">
            <polyline
              ...
              points="0,25 20,20 40,22 60,15 80,18 100,10 120,12 140,8 160,5 180,7 200,3"
              opacity="0.6"
            />
          </svg>
      ...
      {/* Browse by Experience — Zomato-style image cards */}
      <ExperienceCards selected={selectedChip} onSelect={setSelectedChip} />

      {/* Featured Opportunities — Horizontal scroll rich cards */}
  ...
  return (
    ...
    >
      ...
      <div className="relative h-[130px]">
        ...
        {/* Badges */}
        {isHot && (
          <span className="absolute top-2 left-2 badge-hot text-[9px] px-2 py-0.5 rounded-full font-heading font-bold">
            Hot
          </span>
        )}
        {index === 0 && !isHot && (
          <span className="absolute top-2 left-2 badge-new text-[9px] px-2 py-0.5 rounded-full font-heading font-bold">
            New
          </span>
        )}
        {/* Budget badge */}
        <span className="absolute top-2 right-2 bg-emerald-500 text-white text-[10px] font-heading font-bold px-2 py-0.5 rounded-lg">
          {campaign.budget}
        </span>
        {/* Save */}
        <button
      ...
      <div className="p-3">
        <div className="flex items-center gap-2 mb-1.5">
          <div className="w-6 h-6 rounded-lg bg-secondary overflow-hidden flex items-center justify-center shrink-0">
            {campaign.logo?.startsWith("http") ? (
              ...
              <span className="text-[10px] font-heading font-bold text-primary">{campaign.brand.charAt(0)}</span>
            )}
          </div>
        ...
        <div className="flex items-center gap-1.5 mt-2">
          <span className={`text-[9px] font-semibold px-2 py-0.5 rounded-full ${
            compType === "Barter" ? "bg-accent/10 text-accent" : "bg-emerald-500/10 text-emerald-600"
          }`}>
            {compType}
          </span>
